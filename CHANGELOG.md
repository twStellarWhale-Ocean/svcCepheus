...
- **3.1.1 釋出前測試第 2 輪所揭 7 筆修齊**：使用說明之 AI 推論設定改寫本產品實際參數名，並載明關閉思考模式之寫法；主題包連不上時，掛載頁改標「已掛載·無回應」而非「尚未發現」，左欄導覽保留該組並標「暫時無法連線」、頁首改「降級」。
- 單位解除主題套用後，仍停在該包頁面者即被帶回可用頁面，不再看到空資料與按了才知道沒權限的按鈕；拒絕解除套用時列出是哪幾筆未了結事項（類型、代稱、時間），不只給筆數。
- 駕駛回報頁送出後關閉表單，不再因清單重排改綁到另一張派車單；指揮樹編組頁發布失敗時保留錯誤提示與草稿、同一單位同類草稿取代不累積，畫布標明草稿尚未生效。

## 3.0.5 — 2026-09-10

自 `3.0.1` 起 3.0.2、3.0.3、3.0.4 未發 Release，本則合併補記至 `3.0.5`。

- **主題包掛載頁**：「導覽域」欄改顯示中文域名（3.0.2）；卸除前檢查是否使用中——已被單位套用或已有紀錄即擋下並具名列出（3.0.3）；示意圖重畫成現行畫面，徵募站、車隊兩葉元包導覽域改名「徵募站點」「運輸車隊」（3.0.5）。
- **產品內使用說明頁**：改為可導覽——側欄目錄隨捲動標示目前章節、關鍵字搜尋只留命中章節並高亮、表格／程式碼／示意圖完整渲染（3.0.4）；改按需載入、主程式下載量減三成；「關於版本」節補實際內容（3.0.5）。
- **交付**：映像與 chart 皆改為公開 GHCR 套件，安裝不再需要拉取憑證；映像補 OCI 標籤（版本、來源、commit）；四個主題包服務補上自動測試；新增瀏覽器端對端測試。安裝方式與 chart values 不變。

## 3.0.1 — 2026-08-27

自 `2.0.0` 起本檔未同步，本則合併補記 `2.0.0`→`3.0.1` 之對外可見變更。

- **四個主題包成軍**：通用指管、運輸調度，以及徵募站、車隊兩個葉元包（手機優先的接辦與回報）。命令狀態機八態（含無法接辦、中途回報、異常回報、撤銷）；跨包收發依信封受文單位自動列示。
- **權限模型改為「角色×單位」（破壞性，3.0.0）**：主題包只宣告角色（組長／副組長／情報官／管制官／聯絡官／執行員／駕駛），單位由指揮樹給，帳號授權以「單位＋角色」逐對指派。舊版把單位名黏進角色名（`prsn行政組長`）並以人工填的「編制前綴」湊唯一性，前綴一旦不是範例值就會**建得出編制卻派不了人**。**自 2.x 升級須重建資料庫，不設相容層。**
- **維運管理員不是業務角色**：唯一不繫單位的角色，但不因平台權限取得業務紀錄寫入權（職能分離）。
- **單位維護補完**：新增單位須先填名稱；可停用／恢復，停用前檢查下級單位與未終結紀錄。
- **母 chart 廢除（2.1.0）**：改為核心一支、主題包四支各自獨立 release。**主題包之 `core.url` 為必填**——包與核心是不同 release，服務名推不得；未給值即安裝當場報錯，不會裝出一個連不到核心的包（3.0.1）。
- **映像安全**：容器基底升版並移除執行期用不到的 npm CLI，發行前掃描 `HIGH` 以上漏洞為 0。
- **版號單一正本**：版號自 `VERSION.json` 蓋至各處並附機判，杜絕「產品自報舊版」。
- **產品內使用說明**：手冊隨映像內建，不外連原始碼庫。
